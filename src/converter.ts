...
export const convertLinksAndSaveInSingleFile = async (mdFile: TFile, app: App, finalFormat: 'markdown' | 'wiki') => {
...
export const convertLinksInVault = async (app: App, finalFormat: 'markdown' | 'wiki') => {
    ...
    for (let mdFile of mdFiles) {
        ...
        if (
            hasFrontmatter(app, mdFile.path, 'excalidraw-plugin') ||
            hasFrontmatter(app, mdFile.path, 'kanban-plugin')
        ) {
            continue;
        }
        await convertLinksAndSaveInSingleFile(mdFile, app, finalFormat);
